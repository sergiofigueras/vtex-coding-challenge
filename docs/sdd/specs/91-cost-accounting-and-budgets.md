...
Use `config/pricing.openai-2026-09-06.json`, an immutable OpenAI Standard/default-tier snapshot with official model URLs and a review deadline. Never use a transitive adapter's embedded dollar estimate. Add a successor price book rather than editing an already-used one.
...
- **AC-091-03:** Every Harness attempt is attributed to one change; missing usage stays unreconciled and unavailable external usage is never represented as zero.
