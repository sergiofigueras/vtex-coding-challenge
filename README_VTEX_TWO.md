...
> **Pré-requisito para `main`:** esta branch não contém `projects/catalog-consolidation` pronto. Conclua a criação e os testes do consolidador até o passo 6 do [`README_VTEX_ONE.md`](README_VTEX_ONE.md) antes de executar os comandos de instalação ou uso do projeto neste guia. O passo 1 abaixo também aceita o checkout já criado pelo README ONE.
...
- Os sete comandos `project:prepare` do fluxo original e o comando adicional de `SDD-009` estão descritos abaixo; execute-os no seu checkout para criar seus próprios runs.
- Gate raiz: 48 de 48 testes do engine aprovados na verificação local deste guia; repita `npm run check` no seu checkout.
...
O consolidador criado pelo README ONE produz o banco final a partir de duas fontes definidas por hash:
...
| `ProductIdentity` | `ProductId`, `CanonicalVersion`, `CanonicalName`, `CanonicalBrand`, `CanonicalCategory`, `Fingerprint` |
...
| D-007 | Incluir tuplas de seller no hash. | `Fingerprint` não cobre alterações nos sellers. | Uma alteração de seller reindexa somente o produto afetado. |
...
Os comandos do engine já existem no repositório. Os comandos do consolidador só funcionarão após a criação e os testes descritos no README ONE.

### Ordem obrigatória

Os passos desta seção são sequenciais e cada passo pressupõe que os anteriores terminaram com sucesso. Em um checkout novo, comece pelo passo 1, conclua os passos 1 a 6 do README ONE e só então volte ao passo 2 daqui. Não copie um comando de validação isolado para antes das instalações. O `npm ci` raiz instala o engine; o segundo instala o consolidador já criado.

### 1. Clonar e conferir requisitos

Se você já executou o README ONE, reaproveite o mesmo checkout. Caso contrário, o bloco clona a branch que contém este README e o ZIP. Depois do clone novo, volte ao README ONE para criar e testar o consolidador antes do passo 2 deste guia.

```bash
if test ! -d "${REPO_DIR}/.git"; then
  test ! -e "${REPO_DIR}" || { printf 'Diretório existente não é um checkout Git: %s\n' "${REPO_DIR}" >&2; false; }
  git clone \
    --branch main \
    --single-branch \
    --depth 1 \
...
O `--depth 1` apenas reduz o download. A `main` atual foi reescrita como um único commit inicial; os dois commits antigos de upload não fazem parte do histórico alcançável dessa branch.
...
Algumas versões suportadas do Node, como `22.19.0` e `24.0.0`, podem escrever avisos experimentais no `stderr`. Para silenciá-los em um comando direto, passe `--disable-warning=ExperimentalWarning` imediatamente depois de `node`:

```bash
node --disable-warning=ExperimentalWarning caminho/do/script.js [...argumentos]
```
...
A opção silencia somente a categoria `ExperimentalWarning`; ela não faz o Node entender sintaxe TypeScript não suportada. Compile o consolidador e execute `dist/cli.js`, como mostrado na construção do banco final. Outros warnings e mensagens reais da aplicação continuam visíveis.

### 2. Instalar engine e consolidator antes dos gates

Execute o bloco inteiro **depois de concluir o passo 6 do README ONE**. As duas verificações iniciais mostram claramente se o consolidador ou seu lockfile ainda não foram criados. A cadeia `&&` interrompe o restante se alguma etapa falhar:

```bash
test -f "${REPO_DIR}/projects/catalog-consolidation/package.json" &&
test -f "${REPO_DIR}/projects/catalog-consolidation/package-lock.json" &&
...
BUNDLE_SHA256="4226c180bfb5b806f5f876377867a247d240ba6a959546758eedca1aa6d0e217"
...
node --disable-warning=ExperimentalWarning projects/catalog-consolidation/dist/cli.js \
...
Os diretórios em `projects/catalog-rag/.sdd/runs/` são criados localmente quando você executa `project:prepare`. Eles não vêm no ZIP nem no clone limpo. Use os IDs de run impressos pelo comando no seu terminal; um run preparado contém instruções, não código implementado.

## Lista exata de comandos para este checkout

Esta seção continua o setup inicial. Em um checkout novo, não comece aqui: conclua primeiro os passos 1 a 6 de **Comandos para criar o projeto**, incluindo o README ONE, os dois `npm ci`, `project:create`, aplicação do bundle, validação e bootstrap SDD-097. Depois que o bundle tiver sido aplicado, não execute `project:create` nem o descompacte novamente: `projects/catalog-rag` já existirá com as specs validadas.
...
npm --prefix "${REPO_DIR}/projects/catalog-consolidation" run build
test -f "${REPO_DIR}/projects/catalog-consolidation/dist/cli.js"

node --disable-warning=ExperimentalWarning "${REPO_DIR}/projects/catalog-consolidation/dist/cli.js" \
