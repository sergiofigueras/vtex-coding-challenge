...
import { ConsolidationError } from "../src/application/consolidation.ts";
...
test("atomically matches, inserts, and idempotently links seller entries", () => {
  ...
  const first = consolidate(batch, path, false, "run-1"); assert.deepEqual({ matched: first.matchedProducts, products: first.insertedProducts, links: first.insertedLinks }, { matched: 1, products: 1, links: 2 });
  const second = consolidate(batch, path, false, "run-2"); assert.deepEqual({ products: second.insertedProducts, links: second.insertedLinks, present: second.alreadyPresentLinks }, { products: 0, links: 0, present: 2 });
...
test("seller link conflict rolls back the entire batch", () => {
  const path = createDatabase("conflict"); consolidate(input([{ Id: "same", SellerName: "seller", Name: "Café grinder", Brand: "Maker", Category: "Kitchen" }]), path, false, "first");
  assert.throws(() => consolidate(input([{ Id: "new", SellerName: "seller", Name: "Other", Brand: null, Category: "tools" }, { Id: "same", SellerName: "seller", Name: "Different", Brand: null, Category: "tools" }]), path, false, "conflict"), (error: unknown) => error instanceof ConsolidationError && error.code === "seller_link_conflict");
...
test("dry runs roll back migrated schema and planned writes", () => {
  const path = createDatabase("dry"); const result = consolidate(input([{ Id: "one", SellerName: "seller", Name: "New", Brand: null, Category: "tools" }]), path, true, "dry"); assert.equal(result.insertedProducts, 1);
...
test("injected product write failure rolls back migration and the full batch", () => {
  ...
  setup.close();
  assert.throws(() => consolidate(input([{ Id: "one", SellerName: "seller", Name: "New", Brand: null, Category: "tools" }]), path, false, "injected"), /injected product write failure/);
