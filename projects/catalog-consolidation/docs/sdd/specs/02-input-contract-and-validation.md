...
`test/sdd-input-contract.test.mjs` is the acceptance evidence index:

- `AC-002-01` ingests the verified private snapshot and confirms all 269 entries validate, including nullable brands and opaque non-UUID IDs.
- `AC-002-02` verifies trim-scoped seller-entry deduplication, rejects a conflicting reuse before planning, and accepts the same ID for another seller.
- `AC-002-03` table-tests malformed JSON and root, schema, type, required-string, and length failures; it also checks the CLI's bounded, one-based, stack-free diagnostic output.
- `AC-002-04` round-trips quotes, semicolons, SQL comments, Unicode, and control-looking text, then exercises the CLI against SQLite while confirming the existing schema remains intact.

Run `npm run sources:ingest && npm test` to refresh the private fixture and execute the public evidence. `npm run sdd:validate` validates the SDD metadata. `docs/sdd/traceability.json` remains an ownership ledger because its validated schema has no evidence-index field.
