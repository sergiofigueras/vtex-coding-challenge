...
`test/sdd-product-identity.test.mjs` is the acceptance evidence index:

- `AC-004-01` golden-tests pure canonicalization for deterministic normalization and versioning.
- `AC-004-02` covers whitespace, accents, quote variants, punctuation, all reviewed aliases, and a supplied-catalog fixture oracle.
- `AC-004-03` proves injection-shaped display text remains a new literal candidate.
- `AC-004-04` proves duplicate canonical catalog fingerprints return bounded ambiguity instead of selecting a row.
- `AC-004-05` checks the checked-in, versioned alias data with an alias-collapse guard and preserves source display fields.
- `AC-004-06` covers catalog and in-batch potential duplicates, planning abort, and a distinct-model false-merge guard.

Run `npm run sources:ingest && node --test test/sdd-product-identity.test.mjs` for focused evidence, then `npm run check` for the complete project gate. `docs/sdd/traceability.json` remains an ownership ledger because its validated schema has no evidence-index field.
