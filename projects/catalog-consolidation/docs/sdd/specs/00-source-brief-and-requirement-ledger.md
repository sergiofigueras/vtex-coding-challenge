...
Each test name in `test/sdd-source-boundary.test.mjs` identifies its SDD-000 acceptance ID. Run `npm test` to execute all three checks; `npm run sdd:validate` validates the resulting SDD metadata and source-manifest boundary. `docs/sdd/traceability.json` continues to record requirement ownership only because its validated schema has no evidence-index field.
