...
`test/sdd-system-boundary.test.mjs` provides the acceptance evidence index:

- `AC-001-01` validates missing paths and unknown flags exit with code `2` before SQLite planning.
- `AC-001-02` compares all planned count fields for dry-run and normal modes and hashes the temporary SQLite file before and after dry-run.
- `AC-001-03` constructs `ConsolidationService` with in-memory port implementations; it does not start the CLI or create a database.
- `AC-001-04` parses the one-line JSON summary, asserts version `1`, and checks concise text output.

Run `npm test` for this public evidence. `npm run sdd:validate` validates the SDD metadata. The traceability ledger remains ownership-only because its validated schema has no evidence-index field.
