...
- **O que ainda falta comprovar:** a `main` contém as especificações, mas não o aplicativo. O CI atual verifica a estrutura e o ZIP; o comportamento final só estará demonstrado depois que o código for criado e os testes passarem.
...
export BUNDLE_SHA256="0017ba10da2756e5e70974a4c5f6ab437f84cbbb6c9adef512b8701fe50e34c2"
...
Esta é a parte central do desafio. A comparação considera nome, marca e categoria normalizados. Casos parecidos demais para uma decisão segura devem parar para revisão, não virar uma duplicata silenciosa.
...
Rode estes comandos na ordem. A verificação do repositório não substitui os testes do aplicativo. O teste com os dados do exercício usa uma cópia temporária do banco; se algo falhar, corrija o problema e repita a sequência.
...
Faça também uma importação completa à mão. Primeiro copie `catalog.db` para uma pasta temporária: **não escreva no banco original** que foi baixado no passo 3. O primeiro comando da aplicação simula a importação (`--dry-run`); os dois seguintes a executam de verdade, duas vezes.
...
Os dois hashes devem ser iguais. Se o agente criar outro caminho para a CLI, ajuste os comandos acima e documente o caminho usado. Antes de publicar, confira também `git status --short` e `git ls-files`: o PDF do enunciado, o banco, o JSON baixado, chaves e arquivos de `.sdd/runs` não devem entrar no commit.
